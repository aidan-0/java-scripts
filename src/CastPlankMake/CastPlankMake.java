...
@ScriptManifest(name = "Cast Plank Make", description = "Cast Plank Make on Lunar Isle", author = "Luten",
        version = 1.0, category = Category.CONSTRUCTION, image = "")
public class CastPlankMake extends AbstractScript {
...
//    }
    ...
    //need to add equipping of staff of earth and other types
...
//            If restock astrals checkbox is ticked (true) then when astrals run out it will purchase the amount of astrals input by the user from babas shop, before continuing the script
        ...
        } else if (!Inventory.contains("Astral rune") || !Inventory.contains("Nature rune") || !Inventory.contains("Teak logs")) {
            log("State is now WITHDRAW_ITEMS");
            return State.WITHDRAW_ITEMS;

        } else if (Inventory.contains("Astral rune") && Inventory.contains("Nature rune") && Inventory.count("Teak logs") > 0 && bankArea.contains(Players.getLocal().getTile()))  {
            //and air staff equipped
//            log("State is now CAST_PLANK_MAKE");
            return State.CAST_PLANK_MAKE;
        }
        log("returning state");
        return state;
    }
    ...
    public void onPaint(Graphics g) {
        ...
        g.drawString("Planks made: " + planksMade, 35, 70);
        g.drawString("Planks per hour: " + timer.getHourlyRate(planksMade), 35, 90);
